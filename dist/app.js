...
let page=location.hash.slice(1)||'overview',paused=false,frozen=null,range='15m',metric='temperature',search='',severity='ALL',state='ALL',from='',drawer=null,mobile=false,report='',gridlines=true,currentWorkspace=localStorage.getItem('atlas-workspace')||'Groundstation';
...
function overview(){
  ...
  return `<div class="healthband"><div class="healthleft"><div class="healthicon">${icon('shield')}</div><div><h3>${title}</h3><div class="small muted">${subtitle}</div></div></div><div class="healthright"><div class="tiny muted">SYSTEM HEALTH<b>${status(health)}</b></div><div class="tiny muted">ACTIVE DEVICE<b class="mono">${esc(currentWorkspace)}</b></div></div></div>${metricCards()}<div class="grid">${panel('Environment telemetry',`<div class="legend"><span><i></i>${metrics[metric][0]} (${metrics[metric][1]})</span><span class="muted">${service.simulationEnabled?'SIMULATION (DEMO)':service.hasLiveHardware?'LIVE HARDWARE':'AWAITING HARDWARE'}</span></div>${chart(metric)}`,ranges())}${panel('Subsystem health',subsystems(),'<span class="tiny muted">6 SUBSYSTEMS</span>')}</div><div class="grid">${panel('Motion & orientation',motion(),'<span class="tiny muted mono">MPU6050</span>')}${panel('Communication',comms(),icon('radio'))}</div>${recent()}`;
}
...
function content(){
  switch(page){
    ...
    case'devices':{
      const defaultDevs=[
        {deviceId:'Groundstation',name:'Groundstation Base & Telemetry Receiver',status:'NORMAL',mode:service.mode,transport:service.transport,lastSeen:new Date().toISOString()},
        {deviceId:'ATLAS-001',name:'Embedded Monitoring Unit',status:service.health(),mode:service.mode,transport:service.transport,lastSeen:service.last?new Date(service.last).toISOString():'None'}
      ];
      const devList=service.devices.length?service.devices:defaultDevs;
      return panel('Registered devices',`<div class="tablewrap"><table><thead><tr><th>Device</th><th>Status</th><th>Transport</th><th>Last seen</th><th>Mode</th><th>Selection</th></tr></thead><tbody>${devList.map(d=>`<tr><td><b>${esc(d.deviceId)}</b><br><span class="muted">${esc(d.name||'Embedded monitoring unit')}</span></td><td>${status(d.status||service.health())}</td><td><span class="mono">${esc(d.transport||service.transport)}</span></td><td class="mono">${d.lastSeen&&d.lastSeen!=='None'?time(d.lastSeen):'—'}</td><td><span class="chip">${esc(d.mode||service.mode)}</span></td><td><select data-switchdevice aria-label="Device workspace"><option value="${esc(d.deviceId)}" ${d.deviceId===currentWorkspace?'selected':''}>${d.deviceId===currentWorkspace?'Active Workspace':'Select Workspace'}</option><option value="${d.deviceId===currentWorkspace?(d.deviceId==='Groundstation'?'ATLAS-001':'Groundstation'):currentWorkspace}">${d.deviceId===currentWorkspace?(d.deviceId==='Groundstation'?'Switch to ATLAS-001':'Switch to Groundstation'):'Switch to '+esc(d.deviceId)}</option></select></td></tr>`).join('')}</tbody></table></div><p class="small muted">${devList.length} device(s) registered in backend registry. Primary communication: Wi-Fi.</p>`);
    }
...
function bind(){
  ...
  on('workspaceselect','onchange',e=>{
    currentWorkspace=e.target.value;
    try{localStorage.setItem('atlas-workspace',currentWorkspace);}catch{}
    service.setActiveDevice(currentWorkspace);
    render();
  });
  document.querySelectorAll('[data-switchdevice]').forEach(s=>s.onchange=e=>{
    currentWorkspace=e.target.value;
    try{localStorage.setItem('atlas-workspace',currentWorkspace);}catch{}
    service.setActiveDevice(currentWorkspace);
    render();
